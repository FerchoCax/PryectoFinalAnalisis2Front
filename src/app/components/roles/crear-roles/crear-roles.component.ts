...
import { Role, RolesService } from 'src/app/services/api-backend';
...
export class CrearRolesComponent implements OnInit {
  ...
  CrearRol(){
    ...
    this.cargando = true;
    let rolCrear:Role={
	nombreRol:this.rolNuevo,
	usuarioIng:this.autenticationService.userValue.username
    } 
  ...
  desactivarRol(rol:Role){
    Swal.fire({
      title:'Desea desactivar el rol '+rol.nombreRol+' ?',
      icon:'question',
      ...
      if(result.isConfirmed){
        this.servicioRoles.rolesDesactivarRolPost(rol.colRol)
        .subscribe(resultado =>{
